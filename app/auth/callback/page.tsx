        ...
        if (data.session) {
          ...
          setMessage(
            isVendor 
              ...
              : 'Email verified successfully! Thank you for your interest in iOS developers.'
          )
          
          // Redirect to appropriate page after 4 seconds
          setTimeout(() => {
            router.push(isVendor ? '/join?verified=true' : '/')
          }, 4000)
        } else {
          setStatus('error')
          setMessage('No session found. Please check your email link.')
        }
      } catch (err) {
