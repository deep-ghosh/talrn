...
export default function JoinPage() {
  ...
  const validateForm = () => {
    const validation = validateRegistrationForm(formData, password, confirmPassword, accountType)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Sign up user with Supabase (this will send OTP to email)
  ...
  const handleOTPVerify = async (otp: string) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        setMessage('🎉 Email verified successfully! Your vendor application has been submitted.')
      }
    } catch (err) {
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
        <div className="max-w-4xl mx-auto">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <Input
                        ...
                        onChange={(e) => handleInputChange('corporateRegNumber', e.target.value)}
                        className="h-14 rounded-2xl bg-[#F8F9FF] border-[#E1E5F7] text-base px-6 focus:border-[#4F46E5] transition-all"
                        required
                      />
                    </div>
          ...
          {currentStep === 'success' && (
            <div className="max-w-md mx-auto bg-white rounded-3xl shadow-xl p-8 text-center border border-[#E1E5F7]/30">
              ...
              <p className="text-[#6B7280] mb-6 leading-relaxed">
                Welcome to Talrn! Your vendor application has been submitted successfully. 
          ...
          <div className="text-center mt-8 text-sm text-[#6B7280]">
            <p>Already have an account? <a href="/login" className="text-transparent bg-gradient-to-r from-[#4F46E5] to-[#3B82F6] bg-clip-text hover:underline font-medium">Sign in here</a></p>
          </div>
        </div>
      </main>
